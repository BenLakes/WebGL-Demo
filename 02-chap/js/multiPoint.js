/**
 * 目标 使用缓冲区对象 绘制多个点
 * 1. 构建三维模型的基本单位是三角形，不管三维模型的形状多么复杂，其基本组成部分都是三角形
 * 使用 gl.vertexAttrib3f 将点的位置传给变量 a_Position 每次只能传递一个 位置点
 * WebGL 提供了一种很方便的机制,即缓冲区对象(buffer object) 可一次性像顶点着色器传入多个顶点数据
 * 缓冲区对象实际是WebGL中的一块内存区域 可填充大量数据  而后供顶点着色器使用
 */
//顶点着色器代码
let VSHARDER_SOURCE = `
